...
public class UpdateTransactionPostDialog {
	...
	private Node createForm(Transaction transaction) {
		...
		postComboBox.getItems().setAll(postService.getPostsForMonthYear(
				LocalDate.of(transaction.getJournalDate().getYear(), transaction.getJournalDate().getMonth(), 1)));
		...
		Label transactionContraAccountValue = new Label(
				transaction.getContraAccount() == null ? "" : transaction.getContraAccount());
